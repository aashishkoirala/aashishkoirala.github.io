angular.module('akApp', []).controller('akCtrl', function ($scope) {
    ...
    $scope.loadProjects = function () {
        $scope.projects = [{
			...
			description: '(Now obsolete- this was my first attempt at Angular). This is a tool I built to keep track of my personal finances. It lets you create a period budget, and lets you record your transactions so that you can track it against the budget. It is an ASP.NET MVC/Web API SPA web application that uses AngularJS and Bootstrap on the frontend.'
		}, {
			title: 'AK-Login', ghpage: 'http://aashishkoirala.github.io/login/', ghrepo: 'http://github.com/aashishkoirala/login/', application: 'http://aklogin.apphb.com',
			description: 'This is the central login system that I use (or am in the process of migrating them to use) for all of my applications. Once you log in using this application, the idea is for you to be able to navigate to any of my applications without logging in again or having a separate user account. When you navigate to one of my applications and are not logged in (or do not have a user account), you will be redirected to the login page that is hosted by this application. The login page will display information about the target application and will let you login using a number of providers (such as Google or Facebook).'
		}, {
			title: 'Chore', ghpage: 'http://aashishkoirala.github.io/chore/', ghrepo: 'http://github.com/aashishkoirala/chore/', application: 'http://chore.apphb.com',
			description: 'This is a task management application that lets you create and track tasks based on deadlines or recurrence, organize tasks into folders, use built-in or custom filters to look for tasks, get a weekly calendar view based on your tasks, export and import tasks to/from flat data among other things. It is an ASP.NET MVC/Web API SPA web application that uses AngularJS and Bootstrap on the frontend, and uses MongoDB as its database.'
		}, {
