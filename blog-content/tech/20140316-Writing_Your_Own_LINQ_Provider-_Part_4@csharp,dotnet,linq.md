...
1. [A primer](writing-your-own-linq-provider-part-1)
2. [Provider basics](writing-your-own-linq-provider-part-2)
3. [A simple, pointless solution](writing-your-own-linq-provider-part-3)
4. **A tiny ORM of our own** (*this post*)

---

### A tiny ORM of our own
In the [previous post](writing-your-own-linq-provider-part-3), we took a look at a simple, albeit pointless example of a LINQ provider. We wrap the series up this time by looking at something a little less pointless - a LINQ-based ORM, albeit a very rudimentary one. As with the previous one, it helps to take a look at the source code first:
...
If you recall **Step 2** from [provider basics](writing-your-own-linq-provider-part-2), there were two options. The [last solution](writing-your-own-linq-provider-part-3) used Option 1, i.e. there is one queryable that just builds up the expression and the enumerator does the parsing. For this one, we're using Option 2, where we have a separate implementation of `IQueryable<T>` for each type of query operation to support.
