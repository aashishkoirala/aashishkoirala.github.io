...
I guess the only difference with my other application, [Listor](listor-showcasing-react-and-dotnet-core), is that it is a React based SPA with a .NET Core backend. So the build is a little more involved. I therefore found it easier to just write a custom PowerShell script and call that from the build YAML rather than messing around with it in YAML.
