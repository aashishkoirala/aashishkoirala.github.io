...
![Screenshot](tech/finance-manager-screenshot.png)
...
+ [The Commons Library](the-commons-library)- and therefore MEF for DI and extensibility, and [providers](providers-for-the-commons-library-providers) as needed
+ XML file based [configuration](configuration-in-the-commons-library) store provider and text file based [logging](logging-in-the-commons-library) provider
+ A SQL Server database accessed using the Fluent NHibernate [data access](data-access-in-the-commons-library) provider
+ A T4 based [modeling kit](modeling-dsl-and-t4-ramblings) to generate entities and repositories from JSON models
+ Web application based on ASP.NET MVC and a REST API using ASP .NET Web API
+ Frontend based on [TypeScript, AngularJS and Bootstrap](typescript-angularjs-and-bootstrap-the-killer-combo)
