...
4. [A tiny ORM of our own](writing-your-own-linq-provider-part-4)

---

### A simple, pointless solution
In the [previous post](writing-your-own-linq-provider-part-2), we took a look at what happens when you call LINQ methods on `IQueryable<T>`, and how you can use that to build your own provider. We take that a step further this time by building an actual provider - albeit a somewhat pointless one, in that it adds LINQ support to something that doesn't really need it. The point, though, is to keep it simple and try to understand how the process works.
